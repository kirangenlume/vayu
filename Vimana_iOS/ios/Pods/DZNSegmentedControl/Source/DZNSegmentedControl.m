...
- (void)commonInit
{
    ...
    _autoAdjustSelectionIndicatorWidth = NO;
...
- (void)layoutSubviews
{
    ...
    [[self buttons] enumerateObjectsUsingBlock:^(UIButton *button, NSUInteger idx, BOOL *stop) {
        ...
        if (self.numberOfSegments  < 4) {
            ...
            width = self.bounds.size.width/(self.numberOfSegments+2);
            x = width*(idx+1);
        }
        ...
        if(idx!=0){
            ...
            UIView *segmentSeparator = [[UIView alloc] initWithFrame:CGRectMake(button.frame.size.width
                                                                                , 0, 0.3 ,button.frame.size.height-18)];
            //segmentSeparator.backgroundColor= [UIColor colorWithRed:224 green:224 blue:224 alpha:100];
            
            
            segmentSeparator.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.25];
            //            segmentSeparator.opaque = YES;
            
            
            [button addSubview:segmentSeparator];
        }
...
- (CGFloat)height
{
    return (_height ? : self.showsCount ? 70.0f : 20.0f);
}
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            case UIControlStateNormal:              return [UIColor colorWithRed:134/255.0 green:138/255.0 blue:141/255.0 alpha:100];//return [UIColor darkGrayColor];
...
- (CGRect)selectionIndicatorRect
{
    ...
    if (self.autoAdjustSelectionIndicatorWidth) {
        ...
        CGFloat width = button.frame.size.width-15;
        ...
        if (self.numberOfSegments < 4) {
            frame.origin.x = (button.frame.size.width*(self.selectedSegmentIndex+1))+(button.frame.size.width-frame.size.width)/2;
        }
...
- (CGRect)hairlineRect
{
    CGRect frame = CGRectMake(-35.0f, 0.0f, self.superview.frame.size.width+35, 0.5f);
...
- (void)setTitleColor:(UIColor *)color forState:(UIControlState)state
{
    ...
    for (UIButton *button in [self buttons]) {
        ...
        if (self.showsCount) {
            ...
            [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"AktivGrotesk-Thin" size:24.0f] range:[string rangeOfString:count]];
            [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"AktivGrotesk-Regular" size:11.0f] range:[string rangeOfString:title]];
            
            if (state == UIControlStateNormal) {
                
                UIColor *topColor = [UIColor colorWithRed:88/255.0 green:90/255.0 blue:89/255.0 alpha:100];
                UIColor *bottomColor = [UIColor colorWithRed:134/255.0 green:138/255.0 blue:141/255.0 alpha:100];
            ...
            [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"AktivGrotesk-Regular" size:11.0f] range:NSMakeRange(0, attributedString.string.length)];
            UIColor *bottomColor = [UIColor colorWithRed:134/255.0 green:138/255.0 blue:141/255.0 alpha:100];
            [attributedString addAttribute:NSForegroundColorAttributeName value:bottomColor range:NSMakeRange(0, attributedString.string.length)];
